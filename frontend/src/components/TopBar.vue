  ...
  >
    ...
    <div class="relative max-w-sm flex-1">
      ...
      <input
        ...
        class="border-input bg-secondary/60 placeholder:text-muted-foreground focus-visible:ring-ring/50 h-8 w-full rounded-full border pr-4 pl-8 text-sm outline-none focus-visible:ring-3"
        @input="onSearchInput"
      />
    </div>
    ...
    <div class="ml-auto flex items-center gap-4">
      <div class="text-muted-foreground hidden font-mono text-xs sm:block">
        <span class="text-foreground font-medium"
          >{{ totals.healthy }}/{{ totals.count }}</span
        >
        drives ·
      ...
      <Button variant="ghost" size="icon-sm" class="rounded-full" @click="toggle">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger as-child>
          <button
            class="flex items-center gap-2 rounded-full py-1 pr-2 pl-1 transition-colors hover:bg-accent"
          >
            <Avatar class="size-6">
              <AvatarFallback
                class="bg-primary text-primary-foreground font-heading text-[10px]"
              >
                {{ initials }}
              </AvatarFallback>
            </Avatar>
